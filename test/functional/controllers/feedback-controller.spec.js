...
const { test, trait, before } = use('Test/Suite')('Feedback Controller')
...
before(async () => {
  organization = await OrganizationFactory.create({
    slug: 'feedback-controller-test'
  })

  user = await UserFactory.make({
    email: 'feedback-controller-test@email.com',
    password: 'password'
  })
...
test('Make sure user is redirected back after submitting reply', async ({
  ...
  const response = await client
    .post(`organization/${organization.slug}/feedback/${ticket.id}`)
    .send({ reply: 'test' })
    .loginVia(user)
    .end()
